import { useUtils } from '/~/state/utils'

export function getAnagrams(words, wordsLength, min) {
  ...
  do {
    chars = [...getRandomWordEqual(words, wordsLength)]
    anagrams = words.filter((word) => {
      const conditions = [
        [...word].every((v) => chars.includes(v)),
        chars.reduce((arr, v) => {
          if (arr.indexOf(v) !== -1) {
    ...
    attempts++
  } while (attempts < 1000 && anagrams.length < min)
